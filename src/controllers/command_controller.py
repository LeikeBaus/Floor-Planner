"""Command framework for undoable model operations."""
...
class CommandController:
    ...
    def handle_undo_action(self, window: object) -> None:
        """Handle QAction trigger for undo."""
        handler = getattr(window, "_undo", None)
        if callable(handler):
            handler()

    def handle_redo_action(self, window: object) -> None:
        """Handle QAction trigger for redo."""
        handler = getattr(window, "_redo", None)
        if callable(handler):
            handler()

    def handle_delete_action(self, window: object) -> None:
        """Handle QAction trigger for deleting selected objects."""
